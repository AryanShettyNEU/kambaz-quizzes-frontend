...
// 2 & 3. GET SINGLE QUIZ (Handles Admin vs Student view)
export const findQuiz = async (quizId: string, asAdmin: boolean = false) => {
  // If admin (faculty), use the /admin endpoint to get answers
  const path = asAdmin ? `${QUIZZES_API}/${quizId}/admin` : `${QUIZZES_API}/${quizId}`;
