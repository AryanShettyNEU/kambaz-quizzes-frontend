...
import { getAvailabilityMsg } from "./utils";
...
const QuizItem: React.FC<QuizItemProps> = ({ quiz, cid, isFaculty }) => {
  ...
  const handlePublish = async () => {
    await dispatch(
      togglePublish({ quizId: quiz._id, published: !quiz.published })
    );
  ...
  return (
    <ListGroupItem className="wd-quiz p-3 ps-1 d-flex align-items-center">
      <div className="d-flex flex-row">
        <BsRocketTakeoff
          className={`me-3 ms-3 mt-1 fs-5 ${
            quiz.published ? "text-success" : "text-secondary"
          }`}
        />
      </div>
      ...
      <div className="flex-grow-1 d-flex align-items-start">
        <div>
          ...
          <p className="mb-0 text-muted small">
            <span className="fw-bold">{getAvailabilityMsg(quiz)} </span> |&nbsp;
          ...
          <div className="float-end d-flex align-items-center">
            <div onClick={handlePublish} style={{ cursor: "pointer" }}>
              {quiz.published ? (
                ...
                <FaBan className="text-danger fs-5 me-2" />
              )}
            </div>
